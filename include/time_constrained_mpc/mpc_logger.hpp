...
public:
  ...
  bool open(const std::string & filename)
  {
    ...
    // Better: The user of this class should ensure consistency. 
    // For the header, we'll just add a prefix note or handle it dynamically if we want to be strict.
    // Let's just add a flexible number of columns for prediction.
    // Actually, let's just write "pred_sin_0,pred_cos_0,..." for a reasonable max N or just append them.
    // For simplicity in this helper, we'll assume the caller handles the header or we just dump the vector.
    // Let's make the header dynamic based on the first log call? No, header must be first.
    // Let's assume a max N of 50 for the header, or just write a fixed number if N is constant.
    // Since N is a parameter, we can pass it to open().
